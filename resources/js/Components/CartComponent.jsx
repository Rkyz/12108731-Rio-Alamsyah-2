...
const CartComponent = ({handleOpenCart, openCart}) => {
    ...
    const handleDeleteProduct = (product_id) => {
        const updatedCart = cartProducts.filter(product => product.id !== product_id);
        setCartProducts(updatedCart);
        sessionStorage.setItem('products', JSON.stringify(updatedCart));
    };
  ...
  <div>
    ...
    <div className={`fixed flex top-0 w-full max-sm:max-w-full max-sm:flex-col max-sm:z-[90] z-[99] sm:py-[10px] pr-[10px] max-sm:bg-white sm:bg-transparent h-full  right-0 ${openCustomer && cartProducts.length > 0 ? 'sm:max-w-[370px]':'sm:max-w-[330px]'}`}>
            ...
            <div className={`bg-white h-full max-sm:absolute max-sm:z-[90] flex w-full ${openCustomer && 'shadow-[-20px_20px_30px_rgba(0,_0,_0,_0.2)]'}`}>
                ...
                <form onSubmit={handleSaveCustomer} className='p-[15px] flex flex-col gap-[15px]'>
                        <div className='flex flex-col gap-[5px]'>
                            <InputLabel>name Customer</InputLabel>
                            <TextInput required name="name" value={customerData.name} onChange={handleChange} className="w-full"/>
                        </div>
                        <div className='flex flex-col gap-[5px]'>
                            <InputLabel>address Customer</InputLabel>
                            <TextInput required name="address" value={customerData.address} onChange={handleChange} className="w-full"/>
                        </div>
                        <div className='flex flex-col gap-[5px]'>
                            <InputLabel>Nomor Customer</InputLabel>
                            <TextInput required name="no_hp" value={customerData.no_hp} onChange={handleChange} className="w-full"/>
                        </div>
